package app.view.animator
{
	...
	public class Animator extends Sprite
	{
		...
		TweenMax.to(scr, firstTime, { 
							alpha: 1,
							x: (1 - scale) * 0.5 * AppSettings.WIDTH, 
							y: (1 - scale) * 0.5 * AppSettings.WIDTH, 
							ease: Linear.easeIn, 
							scaleX: scale,
							scaleY: scale							
							});
		...
		private function flip():void
		{
			...
			scr.addChild(shadow);
			addChild(scr);
		/*	timeline.append(*/TweenMax.to(shadow, 1.5, { alpha:0.4 } );
		/*	timeline.append(*/TweenMax.to(scr, firstTime, { 
							alpha: 1,
							//y: 300 + shiftY + spaceBettwenPages * (pageNum - 1),								
							x: (1 - scale) * 0.5 * AppSettings.WIDTH, 
							y: (1 - scale) * 0.5 * AppSettings.WIDTH, 
							ease: Linear.easeIn, 
							scaleX: scale,
			...
			/*timeline.append(*/TweenMax.to(scr, 0.8, {delay: firstTime,ease: Linear.easeIn, y: -1750,rotationX: (angle+10),onComplete: function():void
							{								
								removeChild(scr);								
							}})//);
		...
		/*	TweenLite.to(screenshot, 1.5, {x: -400, y: 0, rotationY: -65, ease: Expo.easeOut, alpha: 0.6, onComplete: function():void
				{
					removeChild(screenshot);
				}});*/
		}
