...
package com.modestmaps
{
    ...
    public class TweenMap extends Map
	{
	   ...
	   /**
        ...
        {       	
          ...
          //  trace('mouseWheelingIn', mouseWheelingIn);
           // trace('mouseWheelingOut', mouseWheelingOut); 
			...
			var p1:Point = grid.globalToLocal(new Point(stage.mouseX, stage.mouseY));
			__X = p1.x;
			__Y = p1.y;
		...
		private var minMouseWheelInterval:Number = 500;
		
		public function onKinectWheelOff( ):void
        { 
			__draggable = true;
			grid.kinectDraggable  = true;
		}
		private var __X:Number;
		private var __Y:Number;
		public function onKinectWheel(x:Number,y:Number,delta:Number ):void
        {  
			__draggable = false;
			grid.kinectDraggable = false;
			__X = x;
			__Y = y;
			if (getTimer() - previousWheelEvent < minMouseWheelInterval) return;
			
			previousWheelEvent = getTimer(); 
		
			
			//delta = delta > 0? -1:1;//delta / 1000;
            ...
            if (sc) {
	            var p:Point = grid.globalToLocal(new Point(x, y));        	
				var m:Matrix = grid.getMatrix();
				m.translate(-p.x, -p.y);
				m.scale(sc, sc);
				m.translate(p.x, p.y);
        ...
        protected function doneMouseWheeling():void
        {
          //  var p:Point = grid.globalToLocal(new Point(stage.mouseX, stage.mouseY));
           var p:Point = grid.globalToLocal(new Point(__X,__Y));
